...
# u(0) = 0, 2u'(3) + 2(u(3) - 17/2) = 0, u'(1 - 0) = 2u'(1 + 0) = 4(u(1 + 0) - u(1 - 0))
# Exact solution: u(x) = x^2 on (0, 1), u(x) = x^2/2 + 1 on (1, 3)
...
uexact1 = @(x) x ^ 2;
uexact2 = @(x) x ^ 2 / 2 + 1;
# uexact = @(x) ifelse(x < 1, x ^ 2, x ^ 2 / 2 + 1);

addpath("../../bvp1d");
K = [10, 10];
for test = 0 : 5
  grid_info = get_grid_info(L, K);
  xgrid1 = linspace(0, L(1), K(1) + 1);
  xgrid2 = linspace(L(1), Ltot, K(2) + 1);
  xgrid = [xgrid1, xgrid2];
