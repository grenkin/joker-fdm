...
int main ()
{
    ...
    for (int test = 0; test <= 5; ++test) {
        ...
        for (int n = 0; n <= K0; ++n) {
            ...
            sol[0](0, n) = initfun1(x);
            sol[1](0, n) = initfun2(x);
            for (int i = 0; i < 2; ++i)
                res[i](0, 0, n) = sol[i](0, n);
        }

        data.w[0][0] = w00fun(0.0);
        data.w[1][0] = w10fun(0.0);
        data.w[1][1] = w11fun(0.0);
        ...
        for (int m = 0; m <= tnum; ++m) {
            for (int n = 0; n <= K0; ++n) {
                double x = grid.coord(0, n), t = m * tau;
                rms += pow(res[0](m, 0, n) - exact1(x, t), 2)
                    + pow(res[1](m, 0, n) - exact2(x, t), 2);
            }
        }
